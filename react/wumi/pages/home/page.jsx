...
import { ListView, SearchBar, Grid } from 'antd-mobile';
...
export default class extends Component {
  constructor(props) {
    super(props);
    // 将路由重定向至 /home
    // http:// 协议，www.baidu.com 域名，:3000 端口号， /product 路由 ， /:id?a=1&b=2 参数
  ...
  loadTypeData({ latitude, longitude }) {
    ...
    fetch(`/restapi/shopping/openapi/entries?latitude=$
    {latitude}&longitude=${longitude}&templates[]
    =main_template&templates[]=favourable_template&templates[]=svip_template`)
      .then(res => {
        if (res.status === 200) {
          res.json().then(data => {
            this.setState({
  ...
  loadRestaurantData({ latitude, longitude }, rank_id='') {
    fetch(`/restapi/shopping/v3/restaurants?latitude=${latitude}&
    longitude=${longitude}&offset=${this.rests.length}limit=8&extras[]=activities&extras[]
    =tags&extra_filters=home&rank_id=&terminal=h5&rank_id=${rank_id}`)
      .then(res => {
        if (res.status === 200) {
          res.json().then(data => {
            // concat() 追加数据
            this.rests = [].concat(this.rests, data.items);
            // dataSource.cloneWithRows 克隆（对数据进行深拷贝）
      ...
      <div className={styles.home}>
        ...
        <StickyContainer>
          <Sticky>
            {
              ({style}) => (
                <div style={{...style, zIndex: 1, heigth: '1.2rem', overflow: 'hidden'}}>
